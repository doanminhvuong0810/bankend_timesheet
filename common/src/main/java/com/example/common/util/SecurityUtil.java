...
public class SecurityUtil {
  ...
  private static byte[] encrypt(byte[] plaintext, SecretKeySpec keySpec, byte[] IV) throws Exception {
  ...
  private static String decrypt(byte[] cipherText, SecretKeySpec keySpec, byte[] IV) throws Exception {
    Cipher cipher = Cipher.getInstance(ENCRYPT_ALG);
    GCMParameterSpec gcmParameterSpec = new GCMParameterSpec(GCM_TAG_LENGTH * 8, IV);
